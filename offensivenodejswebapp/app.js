...
GLOBAL.serviceConnectionPoolFileLock = nodeMariaDb.createPool({
    connectionLimit : config.connectionpoolconnectioncount,
    host : config.databasehost,
    port : config.databaseport,
    user : config.databaseusername,
    password : config.databasepassword,
    database : config.databasefilelock/*,
     ssl : {
     ca : fs.readFileSync(config.cert)
     }*/
});

GLOBAL.serviceConnectionPoolFileShare = nodeMariaDb.createPool({
    connectionLimit : config.connectionpoolconnectioncount,
    host : config.databasehost,
    port : config.databaseport,
    user : config.databaseusername,
    password : config.databasepassword,
    database : config.databasefileshare/*,
     ssl : {
     ca : fs.readFileSync(config.cert)
     }*/
});

GLOBAL.serviceConnectionPoolFileVault = nodeMariaDb.createPool({
    connectionLimit : config.connectionpoolconnectioncount,
    host : config.databasehost,
    port : config.databaseport,
    user : config.databaseusername,
    password : config.databasepassword,
    database : config.databasefilevault/*,
     ssl : {
     ca : fs.readFileSync(config.cert)
     }*/
});
