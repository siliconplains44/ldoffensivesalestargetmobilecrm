...
businessObjectsService.factory('businessobjectsservice',
    ['$http', 'utilityservice', 'applicationservice',
        function ($http, utilityService, applicationService) {
            ...
            businessObjectsService.compare = function (a, b) {
                if (a.Name < b.Name)
                    return -1;
                if (a.Name > b.Name)
                    return 1;
            ...
            businessObjectsService.retrieveSalesTargetsWithFilter = function (salesTargetID, securityUserID, salesTargetStageID, teamID, alpha, cb) {
                var self = this;
                ...
                res.success(function (data, status, headers, config) {
                    ...
                    var salestargetsAfterFilter = [];

                    $.each(data.outData.salestargets, function (i, item) {
                        if (item) {
                            if (item.Name) {
                                if (item.Name.match(alpha)) {
                                    salestargetsAfterFilter.push(item);
                                }
                            }
                        }
                    });

                    data.outData.salestargets = salestargetsAfterFilter;

                    data.outData.salestargets.sort(self.compare);
